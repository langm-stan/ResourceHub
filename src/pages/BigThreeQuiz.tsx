...
export function BigThreeQuizContent() {
  ...
  return (
    <div className="mx-auto max-w-3xl border border-stone-200 bg-white p-8 shadow-card">
      {!done ? (
        <>
          <div className="flex items-center gap-2 mb-6">
            {QUESTIONS.map((_, i) => (
              <span key={i} className={`h-2 flex-1 rounded-full ${i <= step ? 'bg-cardinal' : 'bg-stone-200'}`} />
            ))}
          </div>
          <p className="mb-3 text-[16px] font-semibold uppercase tracking-wide text-cardinal">
            Question {step + 1} of {QUESTIONS.length} · {QUESTIONS[step].concept}
          </p>
          <h2 className="mb-6 text-[26px] font-bold leading-snug tracking-[-0.016em] text-stone-900">
            {QUESTIONS[step].q}
          </h2>
          <div className="flex flex-col gap-3 mb-8">
            {QUESTIONS[step].options.map((opt, i) => (
              <button
                key={opt}
                onClick={() => select(i)}
                className={`border px-5 py-4 text-left text-[19px] font-medium transition-colors ${
                  answers[step] === i
          ...
          <button
            ...
            className="flex items-center gap-2 bg-cardinal px-6 py-3 text-[19px] font-semibold text-white transition-colors hover:bg-cardinal-dark disabled:opacity-40"
          >
        ...
        <>
          ...
          <div className="flex flex-col gap-3 mb-6">
            {QUESTIONS.map((q, i) => (
              <div key={q.concept} className="flex items-start gap-3 bg-stone-50 p-4">
                {answers[i] === q.answer ? (
                  <CheckCircle2 className="text-palo-green shrink-0 mt-0.5" size={22} />
                ) : (
                  <XCircle className="mt-0.5 shrink-0 text-cardinal" size={22} />
                )}
                <div>
                  <p className="text-[19px] font-semibold text-stone-900">{q.concept}</p>
                  <p className="text-[17px] text-stone-500">Correct answer: {q.options[q.answer]}</p>
                </div>
              </div>
            ))}
          </div>
          <button
            ...
            className="text-[18px] font-semibold text-cardinal hover:underline"
          >
            Retake the quiz
          </button>
        </>
